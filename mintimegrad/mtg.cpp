...
double beta(double k, double st, double smax) 
{
     ...
     */
    double gamma = 4.257;
    return  sqrt(sqrt((gamma*gamma*smax*smax - k*k*st*st*st*st)
                      *(gamma*gamma*smax*smax - k*k*st*st*st*st)));
}
...
double RungeKutte_rv(double ds, double st, double xpp[], double ypp[], 
    ...
    double k1, k2, k3, k4;
    k1 = ds * (1/st) * sdotdot(xpp[0], ypp[0], zpp[0], xp[0], yp[0], zp[0], st, 
                               smax);

    k2 = ds * (1/(st+k1/2)) * sdotdot(xpp[1], ypp[1], zpp[1], xp[1], yp[1], 
                                      zp[1], st+k1/2, smax);

    k3 = ds * (1/(st+k2/2)) * sdotdot(xpp[1], ypp[1], zpp[1], xp[1], yp[1], 
                                      zp[1], st+k2/2, smax);

    k4 = ds * (1/(st+k3/2)) * sdotdot(xpp[2], ypp[2], zpp[2], xp[2], yp[2],
                                      zp[2], st+k3/2, smax);
...

{
    ...
    for (int i = 0 ; i < half_ls ; i++) {
        ...
        xp[i] = sqrt(
            deriv(length_of_s, s_half[i], s, Csp1x, Csp2x, Csp3x, last) 
            * deriv(length_of_s, s_half[i], s, Csp1x, Csp2x, Csp3x, last)
        );

        yp[i] = sqrt(
            deriv(length_of_s, s_half[i], s, Csp1y, Csp2y, Csp3y, last)
            * deriv(length_of_s, s_half[i], s, Csp1y, Csp2y, Csp3y, last)
        );

        zp[i] = sqrt(
            deriv(length_of_s, s_half[i], s, Csp1z, Csp2z, Csp3z, last)
            * deriv(length_of_s, s_half[i], s, Csp1z, Csp2z, Csp3z, last)
        );
