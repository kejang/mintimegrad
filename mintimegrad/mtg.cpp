...
minTimeGradientRIV: Computes the rotationally invariant solution
    ...
    beta: calculates sqrt (gam^2 * smax^2 - k^2 * st^4) in the ODE.
...
gam: gyromagnetic constant / 2pi [kHz/G]. (4.257 default)
...
Others
------
[Cx Cy Cz]: reparameterized curve, sampled at dt[ms]
    ...
    double smax, double dt, double gam, double ds)
{
    ...
    double *Cx, *Cy, *Cz;
    int l_t = riv_c(&Cx, &Cy, &Cz, x, y, z, Lp, g0, gfin, gmax, smax, dt, gam, ds);
    ...
    {
        gx[i] = (Cx[i + 1] - Cx[i]) / (gam * dt);
        gy[i] = (Cy[i + 1] - Cy[i]) / (gam * dt);
        gz[i] = (Cz[i + 1] - Cz[i]) / (gam * dt);
    }
...

{
    ...
    double *Cx, *Cy, *Cz;
    int l_t = rv_c(&Cx, &Cy, &Cz, x, y, z, Lp, g0, gfin, gmax, smax, dt, gam, ds);
